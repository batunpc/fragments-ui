...
async function init() {
  ...
  async function fragmentEndpoints(e: Event) {
    ...
    if (isError(newFragment)) {
      ...
      console.log(`Returned POST fragment ${JSON.stringify(newFragment)}`);
    }
    ...
    if (isError(fragment)) {
      ...
      console.log(
        `Returned GET fragment ${JSON.stringify(fragment?.fragments)}`
      );
    }
    ...
    console.log(`Returned GET fragment by id${fragmentById}`);
  }
}
